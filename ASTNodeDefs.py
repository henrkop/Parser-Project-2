...
# Class for Declaration statements: type identifier
class Declaration(ASTNode):
    ...
    def to_string(self):
        return f"Declaration({self.var_type}, {self.identifier})"
...
class ForStatement(ASTNode):
    ...
    def to_string(self):
        start_str = (
            ...
            if isinstance(self.start_expr, ASTNode)
            else repr(self.start_expr)
        )
        end_str = (
            self.end_expr.to_string()
            if isinstance(self.end_expr, ASTNode)
            else repr(self.end_expr)
        )
